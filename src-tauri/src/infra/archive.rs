...
pub(crate) fn extract_cover(cbz_path: &Path, cover_id: &str, cache_dir: &Path) -> AppResult<String> {
...
pub(crate) fn count_images(cbz_path: &Path) -> usize {
    fs::File::open(cbz_path)
        ...
        .map(|mut archive| {
            (0..archive.len())
                .filter(|&i| {
                    archive.by_index(i).map(|e| is_image(Path::new(e.name()))).unwrap_or(false)
                })
                .count()
        })
        .unwrap_or(0)
