...
function createContinuousScroll(
...
export function ReaderView() {
  ...
  function cycleReadingMode() {
    ...
    const next =
      READING_MODES[
        (READING_MODES.indexOf(current) + 1) % READING_MODES.length
      ];
    setReadingMode(next);
  ...
  function pageContainerClass() {
    if (anim())
      return `absolute inset-0 flex items-center justify-center overflow-hidden slide-in-${anim()!.dir}`;
    const fm = fitMode();
    const overflow =
  ...
  const visualPrevChapter = () =>
    isRtl() ? state()?.nextChapter : state()?.prevChapter;
  const visualNextChapter = () =>
    isRtl() ? state()?.prevChapter : state()?.nextChapter;
  ...
  function navPrev() {
    return isPaged() ? logicalPrev()() : webtoonScroll("up");
  }
  function navNext() {
    return isPaged() ? logicalNext()() : webtoonScroll("down");
  }
        ...
        <div class="flex flex-1 flex-col overflow-hidden bg-ink-950">
          ...
          <div class="flex shrink-0 items-center gap-2 border-b border-ink-800 bg-ink-900 px-4 py-2.5">
            ...
            <Button
              ...
              title={READING_LABELS[readingMode()]}
            >
              {(() => {
                const Icon = READING_ICONS[readingMode()];
                return <Icon size={14} />;
              })()}
            ...
            <Show when={isPaged()}>
              <Button
                ...
                title={FIT_LABELS[fitMode()]}
              >
                {(() => {
                  const Icon = FIT_ICONS[fitMode()];
                  return <Icon size={14} />;
                })()}
                ...
                >
                  ...
                  >
                    ...
                    <div
                      ...
                      onClick={tapScrollDown}
                    />
                  </div>
                  <div
                    class="flex flex-col items-center"
                    style={{
                      "padding-left": `${webtoonPadding()}%`,
                      "padding-right": `${webtoonPadding()}%`,
                    }}
                  >
                    <Index each={pages()}>
                      {(page, idx) => (
          ...
          <div class="flex shrink-0 items-center justify-center gap-4 border-t border-ink-800 bg-ink-900 px-4 py-2.5">
            <Button
              ...
              disabled={firstChapterDisabled()}
              title={isRtl() ? "Next chapter" : "Previous chapter"}
            >
            ...
            <Button
              ...
              disabled={lastChapterDisabled()}
              title={isRtl() ? "Previous chapter" : "Next chapter"}
            >
              <ChevronLast size={16} />
            </Button>
          </div>
